...
\subsubsection{2.3 Durable Workflow And Multi-Agent Frameworks}\label{durable-workflow-and-multi-agent-frameworks}
...
\subsubsection{2.4 Social Simulation And Agent Memory}\label{social-simulation-and-agent-memory}
...
\subsubsection{2.6 Business Process Management And Process Mining}\label{business-process-management-and-process-mining}
...
\subsubsection{2.7 Institutions, Norms, And Agent Governance}\label{institutions-norms-and-agent-governance}
...
\subsubsection{2.8 Organizational Learning And High-Reliability Work}\label{organizational-learning-and-high-reliability-work}
...
\subsubsection{2.9 Positioning Summary}\label{positioning-summary}
...
\NormalTok{  {-}\textgreater{} message / inbox trigger}
...
\NormalTok{  {-}\textgreater{} inbox state: actionable / superseded / already done}
...
\NormalTok{Tool capability: can see an action{-}capable work surface}
\NormalTok{Role authority: cannot authorize downstream execution}
\NormalTok{Required gate: verifier acceptance and authorized envelope}
\NormalTok{Valid outcome: record needs{-}review or handoff state; message the authorized next owner}
\NormalTok{Invalid outcome: execute because the tool surface made action technically possible}
\end{Highlighting}
\end{Shaded}
...
\subsection{8. Mechanism Case Studies}\label{mechanism-case-studies}
...
\subsubsection{8.2 Case B: Message Trigger Discipline And Stale/No-Reply Suppression}\label{case-b-message-trigger-discipline-and-staleno-reply-suppression}
...
\textbf{Intervention.} Work messages were required to include a work item, exact ask, exact next action, and reply-needed flag. Inbox checks classified messages as actionable, superseded, or already done. Routine replies were suppressed when no recipient needed to act.
...
\subsubsection{8.3 Case C: From Plausible Prose To Verifier-Reconstructible Artifact And Action Gate}\label{case-c-from-plausible-prose-to-verifier-reconstructible-artifact-and-action-gate}
...
\subsubsection{8.4 Case D: Targeted Doctrine Patch Instead Of Whole Rewrite}\label{case-d-targeted-doctrine-patch-instead-of-whole-rewrite}
...
\subsubsection{8.5 Case E: Replay-To-Institutional-Mutation}\label{case-e-replay-to-institutional-mutation}
...
\subsubsection{8.6 Case F: Adaptive Organization Through Role, Tool, Staffing, And Topology Change}\label{case-f-adaptive-organization-through-role-tool-staffing-and-topology-change}
...
\textbf{Intervention.} The organization developed role families, pods, default chains, manager-only durable changes, tool parity checks, and activation discipline.

\textbf{Changed durable layer.} Organization topology, employee/tool lifecycle, role-family contracts, workboard routing.

\textbf{Evidence artifact.} Extracted organization rollout and tool-parity cases show that staffing or tool repair could be the correct institutional fix, not another prompt revision.
...
\subsubsection{8.7 Case G: Replay Without Immediate Durable Mutation}\label{case-g-replay-without-immediate-durable-mutation}
...
\subsubsection{8.8 Case H: Sprint Compilation Into Child Path Work}\label{case-h-sprint-compilation-into-child-path-work}
...
\subsubsection{8.9 Case I: Tool-Parity-Gated Activation}\label{case-i-tool-parity-gated-activation}
...
\textbf{Initial failure.} A topology change could create live employees with partial capability, stale routing truth, or ambiguous activation. A new role could have contract text while still lacking the tool surface or activation path needed for its assigned work.
...
\textbf{Evidence artifact.} Extracted mechanism evidence shows tool-class parity and activation-gating behavior. Sensitive tool details are omitted from public artifacts.

\textbf{General principle.} A role or tool surface is not live institutional capacity until the activation path agrees with the governing routing state.
...
\subsubsection{8.10 Case J: Durable Artifact Growth From Repeated Failure}\label{case-j-durable-artifact-growth-from-repeated-failure}
...
\textbf{Intervention.} Under manager or owner authority, recurring fixes were promoted into durable artifact forms. Repeated role ambiguity became role-family contracts. Repeated case-truth ambiguity became work-record fields. Missed handoffs and false motion became message rules. Plausible but unauthorized artifacts became verifier decision forms. Outcome-learning gaps became replay doctrine. Capability gaps became tool-surface changes.
...
\textbf{Evidence artifact.} The intervention timeline, replay cases, targeted doctrine patching, workboard compilation, message discipline, verifier gates, and tool-parity case all show local failures being routed toward durable artifact changes rather than left as isolated conversation.
...
\textbf{Limitation.} This is mechanism evidence. It does not show autonomous self-improvement or a measured recurrence reduction.
...
\subsubsection{9.3 What Current Agent Approaches Miss}\label{what-current-agent-approaches-miss}
...
\subsubsection{9.4 Practical Design Rules For AI Organizations}\label{practical-design-rules-for-ai-organizations}
...
\subsubsection{9.5 Humanlike Interface, Machine-Native Control}\label{humanlike-interface-machine-native-control}
...
\subsubsection{9.6 Prototype System As Reference Implementation}\label{prototype-system-as-reference-implementation}
...
\subsubsection{9.7 Why The Prediction-Market Desk Is A Useful Critical Case}\label{why-the-prediction-market-desk-is-a-useful-critical-case}
...
\subsubsection{13.2 Included Tables And Appendix Tables}\label{included-tables-and-appendix-tables}
...
\tightlist
\item
  Agent-centric versus organization-centric questions.
\item
  Related-work positioning summary.
\item
  Level hierarchy.
\item
  Organizational primitives and failure modes exposed or controlled.
\item
  Deployment overview and evidence corpus.
\item
  P1/S1/B1 broader spot-check.
\item
  Intervention timeline and 0-to-1 evolution arc.
\item
  Authority lattice and durable instruction layers.
\item
  Common employee runtime contract semantics.
\item
  Metric definitions with data source, status, reported value/window, and limitation.
\item
  Topology-control mechanisms.
\item
  Mechanism case-study summary.
\item
  Product-neutral organization-protocol object model.
\end{enumerate}

Appendix tables include artifact field sets, evidence claim matrix, and evidence-status/coding guide.

\subsubsection{13.3 Appendix A: Role Contract Field Set}\label{appendix-a-role-contract-field-set}
...
\subsubsection{13.4 Appendix B: Work Record Field Set}\label{appendix-b-work-record-field-set}
...
\subsubsection{13.5 Appendix C: Work Message Field Set}\label{appendix-c-work-message-field-set}
...
\subsubsection{13.6 Appendix D: Verifier Decision Field Set}\label{appendix-d-verifier-decision-field-set}
...
\subsubsection{13.7 Appendix E: Common Employee Runtime Contract Semantics}\label{appendix-e-common-employee-runtime-contract-semantics}
...
\subsubsection{13.8 Appendix F: Replay Record Field Set}\label{appendix-f-replay-record-field-set}
...
\subsubsection{13.9 Appendix G: Evidence Claim Matrix}\label{appendix-g-evidence-claim-matrix}
...
\subsubsection{13.10 Appendix H: Evidence Status And Coding Guide}\label{appendix-h-evidence-status-and-coding-guide}
